...
import { BookGroup, reassignmentGroups } from "../../../functions/check_poll_result/utils/group_assignment_v2.ts";
...
Deno.test("그룹 재배정 이후, 최소 인원을 충족하지 못한 그룹은 없어야 한다.", () => {
  const bookGroups: BookGroup[] = [{ bookTitle: '마음', members: ['user1', 'user2', 'user3'] }];
  const min = 4;
